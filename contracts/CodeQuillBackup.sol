...
contract CodeQuillBackupRegistry is Ownable {
    ...
    // repoId => snapshotRoot => single backup
    ...
    /// @dev Only one backup per (repoId, snapshotMerkleRoot) pair. Overwrites existing backup.
    ...
    /// @notice Check if a backup exists for a given (repoId, snapshotRoot).
    ...
    /// @notice Read the backup for a given (repoId, snapshotRoot).
